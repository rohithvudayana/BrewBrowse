      ...
      try {
          ...
          if (existingReview) {
              ...
              const newReviews = await Review.create({
                  reviewID: id,
                  reviewArray: [{ review: newReview, rated: rating }],
...
export const getOverallRating = asyncWrapper (
    async(req: Request, res: Response, next: NextFunction): Promise<void> => {
      ...
      try {
        ...
        if (existingReview) {
          const { reviewArray, overallRating } = existingReview;
          res.status(StatusCodes.OK).json({ overallRating });
        } else {
          res.status(StatusCodes.NOT_FOUND).json({ error: 'Rating not found with the provided ID' });
        }
      } catch (error: any) {
        console.error('Error handling reviews:', error);
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: 'Internal server error' });
